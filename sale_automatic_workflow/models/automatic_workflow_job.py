...
@contextmanager
def savepoint(cr, cron_mode=True):
...
class AutomaticWorkflowJob(models.Model):
    ...
    def _validate_sale_orders(self, order_filter):
        ...
        cron_mode = self._context.get('cron_mode', True)
        for sale in sales:
            with savepoint(self.env.cr, cron_mode), force_company(self.env, sale.company_id):
    ...
    def _validate_invoices(self, validate_invoice_filter):
        ...
        for invoice in invoices:
            with savepoint(self.env.cr, cron_mode), force_company(self.env, invoice.company_id):
    ...
    def _validate_pickings(self, picking_filter):
        ...
        for picking in pickings:
            with savepoint(self.env.cr, cron_mode):
